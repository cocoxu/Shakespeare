...
producing meaning-preserving alternations that preserve the meaning of the input string. [citations] 
...
\begin{enumerate}
  \item Help authors of technical documents to adhere to appropriate stylistic guidelines, or allow writers to automatically “upscale” an email before sending it
  \item Enable non-specialists to better understand information in legal or medical documents by translating ``legalese'' or medical jargon into ordinary English.  
  \item Benefit educational applications:
    \begin{enumerate}
    \item Students studying a specific literary style could benefit from access to \emph{modern English} versions of works by authors they are studying.
    \item Automatic register transformations could help students to experiment with writing in the style of the authors they are studying.
    \end{enumerate}
\end{enumerate}
...
We evaluate these models both through human judgments and standard evaluation metrics from the machine translation and paraphrase literature.  
However, no previous work has investigated the ability of automatic
...
these Elizabethan texts. Among these are “translations” of the plays into colloquial English, and dictionaries that provide modern equivalents for specific words and phrases.
...
additional translations of 8 of these plays from from \url{http://enotes.com}.
...
parallel corpora, resources which may not be available for many writing styles.  For this reason we were motivated to investigate alternative approaches in order to help
...
plays ({\em Romeo and Juliet} was held out for testing) using the SRILM toolkit \cite{Stolcke02}, 
...
\cite{Snow08} to describe videos \cite{chen11}.  We combine a phrase table extracted from this modern, out of domain parallel text, with an in-domain
language model consisting of Shakespeare's 36 plays (again, holding out {\em Romeo and Juliet} for testing), applying the Moses decoder \cite{Koehn07} to find the best paraphrases. 
...
but no corresponding work exists for paraphrasing models that target a specific style of writing.
...
due to the conflation of stylistic differences and semantic adequacy discussed discussed in \S \ref{human_evaluation}.  However it also appears 
...
\section{Conclusions}
We have presented the first investigation into the task of automatic paraphrasing while targeting a specific writing style.  We proposed Shakespeare's plays and their
...
To address this problem, we introduced two new metrics for evaluating writing style, one of which is based on cosine similarity and another based on logistic regression.
