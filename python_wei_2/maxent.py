# Added two functions for 'Classifier Trainer: megam'
# - save trained model to file
# - read model from file
# 
# Usage: replace 'maxent.py' in the nltk package
# e.g. 
# for Mac OS '/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/nltk/classify/maxent.py'
...
# added by Wei Xu (xuwei@cs.nyu.edu)

def train_maxent_classifier_with_megam_writemodeltofile(train_toks, trace=3, encoding=None,
                                       labels=None, gaussian_prior_sigma=0, model_file='megam_model',
                                       **kwargs):
    """
    Train a new ``ConditionalExponentialClassifier``, using the given
    ...
    try:
        ...
        #print 'trainfile_name= %s' % trainfile_name
...
def readin_maxent_classifier_with_megam(train_toks, encoding=None,
                                       labels=None, model_file='megam_model',
                                       **kwargs):
    ...
    print 'model_file= %s' % fin.name
    model = fin.read()
    fin.close()
    
    # Parse the generated weight vector.
    weights = parse_megam_weights(model, encoding.length(), explicit)
